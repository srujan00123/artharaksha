"""
Notifications API
Handles realtime notifications for the Artha Raksha application
"""
...
def get_site_info():
    """
    Get current site information for socket connection
    """
    try:
        return {
        ...
        frappe.log_error(f"Error getting site info: {str(e)}")
        return {
...
def get_user_rooms():
    """
    Get rooms that the current user should join based on their roles
    Uses Frappe's standard room structure
    """
    try:
        user = frappe.session.user
        if user == 'Guest':
        ...
        # Standard Frappe rooms - all users get these
        rooms = ['all', 'website', f'user:{user}']
        
        # Role-based rooms for our specific roles only
        if 'System Manager' in user_roles or 'Administrator' in user_roles:
            rooms.append('admin')
        
        if 'Artha User' in user_roles:
            rooms.append('artha_users')
        
        if 'Insights User' in user_roles:
            rooms.append('insights_users')
        ...
        frappe.log_error(f"Error getting user rooms: {str(e)}")
        return {
            'rooms': ['website'],
            'user': 'Guest',
            'roles': []
...
def send_role_based_notification(roles=None, title=None, message=None, notification_type='info'):
    ...
    try:
        ...
        if not frappe.has_permission('System Settings', 'write'):
            frappe.throw(_("Insufficient permissions to send role-based notifications"))
        ...
        allowed_roles = ['Artha User', 'Insights User', 'System Manager', 'Administrator']
...
def send_room_notification(room=None, title=None, message=None, notification_type='info'):
    ...
    try:
        ...
        if not frappe.has_permission('System Settings', 'write'):
            frappe.throw(_("Insufficient permissions to send room notifications"))
        
        if not room:
            frappe.throw(_("No room specified"))
        
        # Validate room - only allow specific rooms
        allowed_rooms = ['all', 'website', 'admin', 'artha_users', 'insights_users']
...
def send_user_notification(target_user=None, title=None, message=None, notification_type='info'):
    ...
    try:
        ...
        if not frappe.has_permission('System Settings', 'write'):
            frappe.throw(_("Insufficient permissions to send user notifications"))
...
def get_available_rooms():
    ...
    try:
        ...
        user_roles = user_rooms_data.get('roles', [])
