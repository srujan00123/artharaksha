  ...
  <div class="income-management p-3 sm:p-4 lg:p-6 space-y-4 lg:space-y-6">
    ...
    <div v-else class="income-content">
      ...
      <IncomeSources
        ...
        :total-count="getTotalSourcesCount()"
        :monthly-total="dashboardMetrics?.actual_monthly_income || 0"
        :selected-items="selectedItems"
        :loading="loading"
        @add-source="openIncomeForm"
      ...
      <IncomeLedger
        ...
        :total-count="getTotalLedgerCount()"
...
// Helper functions to replace computed properties
const getTotalSourcesCount = () => {
  return filteredSources.value.length
}

const getTotalLedgerCount = () => {
  return filteredLedgerEntries.value.length
}
...
const getTotalCount = () => {
  if (currentView.value === 'sources') {
    return getTotalSourcesCount()
  } else if (currentView.value === 'ledger') {
    return getTotalLedgerCount()
  }
  return 0
}
...
const switchToLedgerView = async () => {
	...
	// Ledger entries are fetched as part of main API, just ensure we have data
