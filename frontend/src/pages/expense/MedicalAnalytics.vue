...
const che10Status = computed(() => {
  ...
  return {
    ...
    progressClass: exceeded ? 'bg-gradient-to-r from-red-400 to-red-600' : 'bg-gradient-to-r from-green-400 to-green-600'
  }
})
...
const che25Status = computed(() => {
  ...
  return {
    ...
    class: exceeded ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800',
    progressClass: exceeded ? 'bg-gradient-to-r from-red-500 to-red-700' : 'bg-gradient-to-r from-yellow-400 to-yellow-600'
  }
})
...
const che40Status = computed(() => {
  ...
  return {
    ...
    class: exceeded ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800',
    progressClass: exceeded ? 'bg-gradient-to-r from-red-600 to-red-800' : 'bg-gradient-to-r from-red-400 to-red-600'
  }
})
...
const monthlyTrendsData = computed((): Array<{
  month: string
  cheRatio: number
  amount: number
}> => {
...
const donutOption = computed(() => {
  const remainingIncome = Math.max(0, totalMonthlyIncome.value - medicalExpenseAmount.value)
...
const getRiskRecommendation = () => {
  ...
  const recommendations = {
    minimal: 'Your medical expenses are within safe limits. Continue monitoring and maintain emergency savings.',
    moderate: 'Medical expenses are approaching concerning levels. Consider reviewing healthcare costs and exploring insurance options.',
    ...
    critical: 'Medical expenses pose critical financial risk. Immediate intervention required to prevent financial hardship.',
...
const getRecommendations = () => {
  ...
  if (level === 'minimal') {
    recommendations.push({
      ...
      description: 'Continue your current approach and build emergency savings for unexpected medical costs.',
      icon: CheckCircle,
      bgClass: 'bg-green-50',
  ...
  if (level === 'moderate') {
    recommendations.push({
      ...
      description: 'Consider exploring health insurance options and preventive care to reduce future medical expenses.',
      icon: Info,
  ...
  if (level === 'high' || level === 'critical') {
    recommendations.push({
      ...
      description: 'Seek immediate assistance through government health schemes, insurance, or financial counseling.',
      icon: XCircle,
      bgClass: 'bg-red-50',
  ...
  recommendations.push({
    ...
    description: 'Check eligibility for Ayushman Bharat, state health insurance, and other welfare programs.',
    icon: Shield,
    bgClass: 'bg-blue-50',
...
watch(expenseFilters, async (newFilters) => {
  await updateFilters(newFilters)
}, { deep: true })
</script>
