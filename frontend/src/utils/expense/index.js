...
export function getCategoryInfo(category, type = 'medical') {
    const categories = type === 'medical' ? MEDICAL_EXPENSE_CATEGORIES : OTHER_EXPENSE_CATEGORIES
...
export function getExpenseSummary(expenses) {
    ...
    const medicalTotal = medical.reduce((sum, e) => sum + (parseFloat(e.amount) || 0), 0)
    const otherTotal = other.reduce((sum, e) => sum + (parseFloat(e.amount) || 0), 0)
...
export const INSURANCE_SCHEMES = {
    pmjay: { name: 'PM-JAY (Ayushman Bharat)', coverage: 500000, type: 'government' },
    esis: { name: 'ESIS', coverage: 100000, type: 'government' },
    private: { name: 'Private Health Insurance', coverage: 1000000, type: 'private' },
    employer: { name: 'Employer Health Insurance', coverage: 500000, type: 'employer' }
}
