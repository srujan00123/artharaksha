...
export const useSupportStore = defineStore('support', {
    ...
    getters: {
        ...
        getHealthConditionsByType: (state) => (type: string): ProcessedHealthCondition[] => {
            return state.healthConditions.filter(condition =>
                condition.condition_type === type
            )
        },

        getHealthConditionsByName: (state) => (name: string): ProcessedHealthCondition | undefined => {
            return state.healthConditions.find(condition =>
                condition.name === name
            )
        },
        ...
        getGovernmentWelfareSchemes: (state): ProcessedWelfareScheme[] => {
            return state.welfareSchemes.filter(scheme => scheme.type === 'Government')
        },
        ...
        getGovernmentInsuranceSchemes: (state): ProcessedInsuranceScheme[] => {
            return state.insuranceSchemes.filter(scheme => scheme.type === 'Government')
        },

        getPrivateInsuranceSchemes: (state): ProcessedInsuranceScheme[] => {
            return state.insuranceSchemes.filter(scheme => scheme.type === 'Private')
        },

        // All Schemes getters
        getGovernmentSchemes: (state): (ProcessedWelfareScheme | ProcessedInsuranceScheme)[] => {
            return state.allSchemes.filter(scheme => scheme.type === 'Government')
        },

        getPrivateSchemes: (state): (ProcessedWelfareScheme | ProcessedInsuranceScheme)[] => {
            return state.allSchemes.filter(scheme => scheme.type === 'Private')
        },

        getWelfareSchemes: (state): (ProcessedWelfareScheme | ProcessedInsuranceScheme)[] => {
            return state.allSchemes.filter(scheme =>
                'scheme_source' in scheme ? scheme.scheme_source === 'welfare' : true
            )
        },

        getInsuranceSchemes: (state): (ProcessedWelfareScheme | ProcessedInsuranceScheme)[] => {
            return state.allSchemes.filter(scheme =>
                'scheme_source' in scheme ? scheme.scheme_source === 'insurance' : false
            )
        },

        getEligibleGovernmentSchemes: (state): EligibilityResult[] => {
            return state.eligibleSchemes.filter(scheme =>
                scheme.type === 'Government' && scheme.is_eligible
            )
        },

        getPartiallyEligibleSchemes: (state): EligibilityResult[] => {
            return state.eligibleSchemes.filter(scheme =>
                !scheme.is_eligible && scheme.eligibility_score > 0
            )
        },

        // Support Pathways getters
        getSupportPathwaysByTitle: (state) => (title: string): ProcessedSupportPathway | undefined => {
            return state.supportPathways.find(pathway =>
                pathway.title && pathway.title.toLowerCase().includes(title.toLowerCase())
            )
        },
        ...
        totalRecommendations: (state): number => {
            const { health_based_support, income_based_support, insurance_recommendations, general_support } = state.supportRecommendations
            return health_based_support.length + income_based_support.length + insurance_recommendations.length + general_support.length
        },

        isDataStale: (state) => (dataType: keyof SupportStoreState['lastFetched'], maxAge: number = 5 * 60 * 1000): boolean => { // 5 minutes default
    ...
    actions: {
        ...
        async fetchHealthConditions(forceRefresh: boolean = false): Promise<ProcessedHealthCondition[]> {
            this.healthConditionsLoading = true
            this.healthConditionsError = ''

            try {
                const conditions = await supportService.getHealthConditions(true, forceRefresh)
                ...
                this.healthConditionsError = error instanceof Error ? error.message : 'Failed to load health conditions'
                return []
            } finally {
        ...
        async fetchWelfareSchemes(filters: SupportFilters | null = null, forceRefresh: boolean = false): Promise<ProcessedWelfareScheme[]> {
            this.welfareSchemesLoading = true
            this.welfareSchemesError = ''

            try {
                const schemes = await supportService.getWelfareSchemes(filters, { useCache: true, forceReload: forceRefresh })
                ...
                this.welfareSchemesError = error instanceof Error ? error.message : 'Failed to load welfare schemes'
                return []
            } finally {
        ...
        async fetchInsuranceSchemes(filters: SupportFilters | null = null, forceRefresh: boolean = false): Promise<ProcessedInsuranceScheme[]> {
            this.insuranceSchemesLoading = true
            this.insuranceSchemesError = ''

            try {
                const schemes = await supportService.getInsuranceSchemes(filters, { useCache: true, forceReload: forceRefresh })
                ...
                this.insuranceSchemesError = error instanceof Error ? error.message : 'Failed to load insurance schemes'
                return []
            } finally {
        ...
        // All Schemes actions
        async fetchAllSchemes(filters: SupportFilters | null = null, forceRefresh: boolean = false): Promise<(ProcessedWelfareScheme | ProcessedInsuranceScheme)[]> {
            this.allSchemesLoading = true
            this.allSchemesError = ''

            try {
                const schemes = await supportService.getAllSchemes(filters, { useCache: true, forceReload: forceRefresh })
                ...
                this.allSchemesError = error instanceof Error ? error.message : 'Failed to load schemes'
                return []
            } finally {
        ...
        async fetchSupportPathways(filters: SupportFilters | null = null, forceRefresh: boolean = false): Promise<ProcessedSupportPathway[]> {
            this.supportPathwaysLoading = true
            this.supportPathwaysError = ''

            try {
                const pathways = await supportService.getSupportPathways(filters, { useCache: true, forceReload: forceRefresh })
                ...
                this.supportPathwaysError = error instanceof Error ? error.message : 'Failed to load support pathways'
                return []
            } finally {
        ...
        async fetchHouseholdProfile(forceRefresh: boolean = false): Promise<ProcessedHouseholdProfile | null> {
            this.householdProfileLoading = true
            this.householdProfileError = ''

            try {
                const profile = await supportService.getHouseholdProfile({ useCache: true, forceReload: forceRefresh })
                ...
                this.householdProfileError = error instanceof Error ? error.message : 'Failed to load household profile'
                return null
            } finally {
        ...
        async updateHealthConditions(healthConditions: ProcessedHealthCondition[]): Promise<void> {
            try {
                ...
                console.error('SupportStore: Failed to update health conditions:', error)
                throw error
            }
        },

        // Eligible Schemes actions
        async fetchEligibleSchemes(forceRefresh: boolean = false): Promise<EligibilityResult[]> {
            this.eligibleSchemesLoading = true
            this.eligibleSchemesError = ''

            try {
                const schemes = await supportService.getEligibleSchemes({ useCache: true, forceReload: forceRefresh })
                ...
                this.eligibleSchemesError = error instanceof Error ? error.message : 'Failed to load eligible schemes'
                return []
            } finally {
        ...
        async fetchSupportRecommendations(forceRefresh: boolean = false): Promise<SupportRecommendations> {
            this.supportRecommendationsLoading = true
            this.supportRecommendationsError = ''

            try {
                const recommendations = await supportService.getSupportRecommendations({ useCache: true, forceReload: forceRefresh })
                ...
                console.error('SupportStore: Failed to fetch support recommendations:', error)
                this.supportRecommendationsError = error instanceof Error ? error.message : 'Failed to load support recommendations'
                return {
                    health_based_support: [],
                    income_based_support: [],
                    insurance_recommendations: [],
        ...
        async fetchApplications(forceRefresh: boolean = false): Promise<SchemeApplication[]> {
            this.applicationsLoading = true
            this.applicationsError = ''

            try {
                const applications = await supportService.getSchemeApplications({ useCache: true, forceReload: forceRefresh })
                this.applications = applications
                this.lastFetched.applications = Date.now()
                return this.applications
            } catch (error) {
                console.error('SupportStore: Failed to fetch applications:', error)
                this.applicationsError = error instanceof Error ? error.message : 'Failed to load applications'
                return []
            } finally {
        ...
        async createApplication(applicationData: ApplicationFormData): Promise<SchemeApplication | null> {
            try {
                const newApplication = await supportService.createSchemeApplication(applicationData)
        ...
        async updateApplication(applicationName: string, applicationData: Partial<ApplicationFormData>): Promise<SchemeApplication | null> {
            try {
                const updatedApplication = await supportService.updateSchemeApplication(applicationName, applicationData)
                const index = this.applications.findIndex(app => app.name === applicationName)
                if (index !== -1) {
        ...
        // Claims actions
        async fetchClaims(forceRefresh: boolean = false): Promise<SchemeClaim[]> {
            this.claimsLoading = true
            this.claimsError = ''

            try {
                const claims = await supportService.getSchemeClaims({ useCache: true, forceReload: forceRefresh })
                this.claims = claims
                this.lastFetched.claims = Date.now()
                return this.claims
            } catch (error) {
                console.error('SupportStore: Failed to fetch claims:', error)
                this.claimsError = error instanceof Error ? error.message : 'Failed to load claims'
                return []
            } finally {
        ...
        async updateClaim(claimName: string, claimData: Partial<ClaimFormData>): Promise<SchemeClaim | null> {
            try {
                const updatedClaim = await supportService.updateSchemeClaim(claimName, claimData)
        ...
        setInsuranceSchemeFilters(filters: Partial<SupportFilters>): void {
            this.insuranceSchemeFilters = { ...this.insuranceSchemeFilters, ...filters }
        },
        ...
        // Cache management
        clearCache(dataType: keyof SupportStoreState['lastFetched'] | null = null): void {
            if (dataType) {
