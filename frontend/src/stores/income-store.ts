 ...
 * Follows the expense store pattern exactly
 */
...
export const useIncomeStore = defineStore("income", () => {
  ...
  // Computed properties following expense store pattern
  ...
  const recurringIncome = computed(
    () => analytics.value?.recurring_income || 0,
  );
  ...
  // CRUD operations with consistent pattern
  ...
  async function refreshIncome(): Promise<void> {
    clearCache();
    await fetchIncomeWithAnalytics({ forceRefresh: true });
  }
