...
export const useExpenseStore = defineStore('expense', {
    ...
    getters: {
        ...
        recentExpenses: (state): ProcessedExpenseItem[] => {
            ...
            return state.processedExpenses.filter(expense =>
         ...
         */
        topCategories: (state): Array<{ category: string; amount: number; count: number }> => {
        ...
        expenseSummary: (state): ExpenseSummary => {
            ...
            return {
                ...
                averageExpense: filtered.length > 0 ?
                    filtered.reduce((sum, exp) => sum + exp.amount, 0) / filtered.length : 0,
        ...
        filterSummary: (state): string => {
            ...
            return activeFilters.length > 0 ? activeFilters.join(', ') : 'All expenses'
        },
    ...
    actions: {
         ...
         * Create new expense
         */
        async createExpense(expenseData: ExpenseFormData): Promise<ProcessedExpenseItem | null> {
            this.loading = true
            this.error = ''
            ...
            try {
                ...
                this.error = error instanceof Error ? error.message : 'Failed to create expense'
                return null
            } finally {
                this.loading = false
            }
         ...
         */
        async updateExpense(originalExpense: ProcessedExpenseItem, updatedData: ExpenseFormData): Promise<ProcessedExpenseItem | null> {
            this.loading = true
            this.error = ''

            try {
                const updatedExpense = await expenseService.updateExpense(originalExpense, updatedData)

                // Update in local state
                const index = this.processedExpenses.findIndex(exp => exp.id === originalExpense.id)
                if (index !== -1) {
        ...
        async deleteExpense(expense: ProcessedExpenseItem): Promise<boolean> {
            ...
            try {
                ...
                const index = this.processedExpenses.findIndex(exp => exp.id === expense.id)
                if (index !== -1) {
        ...
        applyFilters(): void {
            ...
            filtered = expenseService.filterExpenses(this.processedExpenses, this.filters)
         ...
         */
        async loadAnalytics(period: string = 'last_3_months'): Promise<void> {
            try {
                this.analytics = await expenseService.getExpenseAnalytics(period, this.filters)
            } catch (error) {
