...
interface Notification {
    ...
    title: string
    message: string
    type: 'success' | 'error' | 'warning' | 'info' | 'health' | 'income' | 'expense' | 'support'
    timestamp: Date
    read: boolean
    data?: any
}
...
export function useNotifications() {
    ...
    const connectionError = computed(() => globalNotificationState.connectionError)
    ...
    const setupRealtimeListeners = () => {
        ...
        socketClient.on('che_alert', (data: any) => {
            addNotification({
                title: 'CHE Alert',
                message: data.message || 'Catastrophic Health Expenditure threshold reached',
                type: 'warning',
                data
            })
        })
        ...
        socketClient.on('connect_error', (error: any) => {
            globalNotificationState.connectionError = error.message || 'Connection error'
    ...
    const addNotification = (notification: Omit<Notification, 'id' | 'timestamp' | 'read'>) => {
        ...
        if (globalNotificationState.notifications.length > 50) {
            globalNotificationState.notifications = globalNotificationState.notifications.slice(0, 50)
        }
    ...
    const markAsRead = (id: string) => {
        const notification = globalNotificationState.notifications.find(n => n.id === id)
        if (notification) {
