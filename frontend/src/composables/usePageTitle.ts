...
const sectionDescriptions: Record<string, string> = {
    'Dashboard': 'Healthcare financial protection dashboard with expense tracking and welfare scheme management',
    'IncomeManagement': 'Manage and track your income sources for comprehensive financial planning',
    'IncomeReports': 'Detailed income analysis and reporting for financial insights',
    'ExpenseOverview': 'Track and analyze your medical and healthcare expenses',
    'ExpenseAnalytics': 'Comprehensive healthcare expense analytics with CHE analysis',
    ...
    'SchemesAndPrograms': 'Discover and apply for welfare schemes and insurance programs',
...
export function usePageTitle(): UsePageTitleReturn {
    ...
    const getTitleForRoute = (routeName: string | symbol | null | undefined): string => {
    ...
    watch(
        ...
        (newRoute: RouteLocationNormalized) => {
            if (newRoute) {
                ...
                const pageTitle = newRoute.meta?.title as string || getTitleForRoute(newRoute.name)
                setTitle(pageTitle)

                // Use route meta description if available
                const routeName = newRoute.name
                const description = (newRoute.meta?.description as string) ||
                    (typeof routeName === 'string' ? sectionDescriptions[routeName] : undefined)
                if (description) {
                    updateMetaDescription(description)
                }
            }
