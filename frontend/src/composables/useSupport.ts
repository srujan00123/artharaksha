...
export function useSupport(options: UseSupportOptions = {}) {
    ...
    const loading = computed(() => ({
        ...
        any: store.healthConditionsLoading || store.welfareSchemesLoading ||
            store.insuranceSchemesLoading || store.allSchemesLoading ||
            store.supportPathwaysLoading || store.householdProfileLoading ||
            store.eligibleSchemesLoading || store.supportRecommendationsLoading ||
            store.applicationsLoading || store.claimsLoading
    }))
    ...
    const errors = computed(() => ({
        ...
        any: store.healthConditionsError || store.welfareSchemesError ||
            store.insuranceSchemesError || store.allSchemesError ||
            store.supportPathwaysError || store.householdProfileError ||
            store.eligibleSchemesError || store.supportRecommendationsError ||
            store.applicationsError || store.claimsError
    }))
    ...
    const invalidateCache = (dataType?: string): void => {
        ...
        if (dataType) {
            ...
            if (dataType === 'healthConditions' || dataType === 'userHealthConditions') {
    ...
    const loadHealthConditions = async (forceRefresh: boolean = false): Promise<ProcessedHealthCondition[]> => {
        try {
            if (forceRefresh) {
            ...
            state.error = error instanceof Error ? error.message : 'Failed to load health conditions'
            throw error
        }
    }

    const loadWelfareSchemes = async (filters?: SupportFilters, forceRefresh: boolean = false): Promise<ProcessedWelfareScheme[]> => {
        try {
            if (filters) {
            ...
            state.error = error instanceof Error ? error.message : 'Failed to load welfare schemes'
            throw error
        }
    }

    const loadInsuranceSchemes = async (filters?: SupportFilters, forceRefresh: boolean = false): Promise<ProcessedInsuranceScheme[]> => {
        try {
            if (filters) {
            ...
            state.error = error instanceof Error ? error.message : 'Failed to load insurance schemes'
            throw error
        }
    }

    const loadAllSchemes = async (filters?: SupportFilters, forceRefresh: boolean = false): Promise<(ProcessedWelfareScheme | ProcessedInsuranceScheme)[]> => {
        try {
            if (filters) {
            ...
            state.error = error instanceof Error ? error.message : 'Failed to load schemes'
            throw error
        }
    }

    const loadSupportPathways = async (filters?: SupportFilters, forceRefresh: boolean = false): Promise<ProcessedSupportPathway[]> => {
        try {
            if (forceRefresh) {
            ...
            state.error = error instanceof Error ? error.message : 'Failed to load support pathways'
            throw error
        }
    }

    const loadHouseholdProfile = async (forceRefresh: boolean = false): Promise<ProcessedHouseholdProfile | null> => {
        try {
            if (forceRefresh) {
            ...
            state.error = error instanceof Error ? error.message : 'Failed to load household profile'
            throw error
        }
    }

    const loadEligibleSchemes = async (forceRefresh: boolean = false): Promise<EligibilityResult[]> => {
        try {
            if (forceRefresh) {
            ...
            state.error = error instanceof Error ? error.message : 'Failed to load eligible schemes'
            throw error
        }
    }

    const loadSupportRecommendations = async (forceRefresh: boolean = false): Promise<SupportRecommendations> => {
        try {
            if (forceRefresh) {
            ...
            state.error = error instanceof Error ? error.message : 'Failed to load support recommendations'
            throw error
        }
    }

    const loadApplications = async (forceRefresh: boolean = false): Promise<SchemeApplication[]> => {
        try {
            if (forceRefresh) {
            ...
            state.error = error instanceof Error ? error.message : 'Failed to load applications'
            throw error
        }
    }

    const loadClaims = async (forceRefresh: boolean = false): Promise<SchemeClaim[]> => {
        try {
            if (forceRefresh) {
            ...
            state.error = error instanceof Error ? error.message : 'Failed to load claims'
            throw error
        }
    }

    // Refresh methods (force refresh with cache invalidation)
    const refreshHealthConditions = async (): Promise<ProcessedHealthCondition[]> => {
        return await loadHealthConditions(true)
    }

    const refreshWelfareSchemes = async (filters?: SupportFilters): Promise<ProcessedWelfareScheme[]> => {
        return await loadWelfareSchemes(filters, true)
    }

    const refreshInsuranceSchemes = async (filters?: SupportFilters): Promise<ProcessedInsuranceScheme[]> => {
        return await loadInsuranceSchemes(filters, true)
    }

    const refreshAllSchemes = async (filters?: SupportFilters): Promise<(ProcessedWelfareScheme | ProcessedInsuranceScheme)[]> => {
        return await loadAllSchemes(filters, true)
    }

    const refreshSupportPathways = async (filters?: SupportFilters): Promise<ProcessedSupportPathway[]> => {
        return await loadSupportPathways(filters, true)
    }

    const refreshHouseholdProfile = async (): Promise<ProcessedHouseholdProfile | null> => {
    ...
    const refreshSupportRecommendations = async (): Promise<SupportRecommendations> => {
    ...
    const updateHealthConditions = async (conditions: ProcessedHealthCondition[]): Promise<void> => {
        try {
            ...
            state.error = error instanceof Error ? error.message : 'Failed to update health conditions'
            throw error
        }
    }

    const createApplication = async (applicationData: ApplicationFormData): Promise<SchemeApplication | null> => {
        try {
    ...
    const updateApplication = async (applicationName: string, applicationData: Partial<ApplicationFormData>): Promise<SchemeApplication | null> => {
        try {
            const result = await store.updateApplication(applicationName, applicationData)
            invalidateCache('applications')
            return result
        } catch (error) {
    ...
    const createClaim = async (claimData: ClaimFormData): Promise<SchemeClaim | null> => {
        try {
            const result = await store.createClaim(claimData)
            invalidateCache('claims')
            return result
        } catch (error) {
            state.error = error instanceof Error ? error.message : 'Failed to create claim'
            throw error
        }
    }

    const updateClaim = async (claimName: string, claimData: Partial<ClaimFormData>): Promise<SchemeClaim | null> => {
        try {
            const result = await store.updateClaim(claimName, claimData)
            invalidateCache('claims')
            return result
        } catch (error) {
            state.error = error instanceof Error ? error.message : 'Failed to update claim'
            throw error
        }
    }

    // Check if data is stale
    const isDataStale = (dataType: string, maxAge: number = cacheTimeout): boolean => {
    ...
    const initialize = async (): Promise<void> => {
        ...
        try {
            ...
            state.error = error instanceof Error ? error.message : 'Failed to initialize support system'
