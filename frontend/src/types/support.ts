...
export const SEVERITY_LEVELS = [
    { value: 'Mild', label: 'Mild', color: 'green', description: 'Minor impact on daily activities' },
    { value: 'Moderate', label: 'Moderate', color: 'yellow', description: 'Some impact on daily activities' },
    { value: 'Severe', label: 'Severe', color: 'red', description: 'Significant impact on daily activities' }
] as const

export const DURATION_TYPES = [
    { value: 'Temporary', label: 'Temporary', description: 'Expected to resolve over time' },
    { value: 'Permanent', label: 'Permanent', description: 'Long-term or chronic condition' }
] as const

export const SCHEME_TYPES = [
    { value: 'Government', label: 'Government', icon: '🏛️', description: 'Government-sponsored schemes' },
    { value: 'Private', label: 'Private', icon: '🏢', description: 'Private organization schemes' }
] as const

export const APPLICATION_STATUS_OPTIONS = [
    { value: 'pending', label: 'Pending', color: 'yellow', description: 'Application is under review' },
    { value: 'approved', label: 'Approved', color: 'green', description: 'Application has been approved' },
    { value: 'rejected', label: 'Rejected', color: 'red', description: 'Application has been rejected' }
] as const

export const CLAIM_STATUS_OPTIONS = [
    { value: 'submitted', label: 'Submitted', color: 'blue', description: 'Claim has been submitted' },
    { value: 'processing', label: 'Processing', color: 'yellow', description: 'Claim is being processed' },
    { value: 'approved', label: 'Approved', color: 'green', description: 'Claim has been approved' },
    { value: 'rejected', label: 'Rejected', color: 'red', description: 'Claim has been rejected' },
    { value: 'paid', label: 'Paid', color: 'green', description: 'Claim amount has been paid' }
] as const
...
export const RISK_THRESHOLDS = {
    ...
    HIGH: {
        maxHealthConditions: Infinity,
        minAnnualIncome: 0,
        maxCHEIndicators: Infinity
    }
} as const

// Utility functions
export function calculateRiskLevel(profile: BackendHouseholdProfile): 'low' | 'medium' | 'high' {
...
export function calculateProfileCompleteness(profile: BackendHouseholdProfile): number {
    if (!profile) return 0
...
export function getSeverityColor(severity: string): string {
    const severityLevel = SEVERITY_LEVELS.find(level => level.value === severity)
...
export function getClaimStatusColor(status: ClaimStatus): string {
    const statusOption = CLAIM_STATUS_OPTIONS.find(option => option.value === status)
    return statusOption?.color || 'gray'
}

export function getStatusLabel(status: ApplicationStatus | ClaimStatus): string {
    // Check application status first
    const applicationStatus = APPLICATION_STATUS_OPTIONS.find(option => option.value === status)
    if (applicationStatus) {
        return applicationStatus.label
    }

    // Check claim status
    const claimStatus = CLAIM_STATUS_OPTIONS.find(option => option.value === status)
    if (claimStatus) {
...
export function getSchemeDisplayName(item: SchemeApplication | SchemeClaim): string {
...
export function getCoverageAmount(application: SchemeApplication): number | null {
...
// Type guards
export function isProcessedHealthCondition(obj: any): obj is ProcessedHealthCondition {
    return obj && typeof obj === 'object' && 'condition_name' in obj
}

export function isProcessedWelfareScheme(obj: any): obj is ProcessedWelfareScheme {
    return obj && typeof obj === 'object' && 'scheme_name' in obj
}

export function isProcessedSupportPathway(obj: any): obj is ProcessedSupportPathway {
    return obj && typeof obj === 'object' && 'title' in obj
}

export function isProcessedHouseholdProfile(obj: any): obj is ProcessedHouseholdProfile {
    return obj && typeof obj === 'object' && 'user' in obj
}

export function isSchemeApplication(obj: any): obj is SchemeApplication {
    return obj && typeof obj === 'object' && ('scheme_reference' in obj || 'custom_scheme' in obj)
}

export function isSchemeClaim(obj: any): obj is SchemeClaim {
    return obj && typeof obj === 'object' && ('scheme_reference' in obj || 'custom_scheme' in obj) && 'claim_amount' in obj
}
