 ...
 */
const transformExpenseDocument = (
	doc: BackendExpense,
): ProcessedExpenseItem[] => {
		...
		doc.medical_expenses.forEach((medExpense, index) => {
			if (isMedicalExpenseType(medExpense)) {
				expenses.push({
					...
					description:
						medExpense.description || medExpense.medical_expense_type || "",
					amount: Number.parseFloat(String(medExpense.amount || 0)),
		...
		doc.other_expenses.forEach((otherExpense, index) => {
			if (isExpenseType(otherExpense)) {
				expenses.push({
					...
					description:
						otherExpense.description || otherExpense.expense_type || "",
					amount: Number.parseFloat(String(otherExpense.amount || 0)),
...
): Partial<BackendMedicalExpenseType | BackendExpenseType> => {
...
export class ExpenseService {
		...
		try {
			...
			for (const doc of expenseDocuments) {
				try {
					...
					console.warn(
						`Failed to load complete document ${doc.name}:`,
						docError,
					)
		...
		try {
			...
			if (existingDocs.length > 0) {
				...
				if (expenseData.type === "medical") {
					...
					expenseDoc.medical_expenses.push(
						backendData as BackendMedicalExpenseType,
					)
				} else {
				...
				if (expenseData.type === "medical") {
					documentResource.doc.medical_expenses = [
						backendData as BackendMedicalExpenseType,
					]
				} else {
					documentResource.doc.other_expenses = [
						backendData as BackendExpenseType,
					]
				}
	...
	async updateExpense(
		...
		try {
			...
			const updatedExpense = transformedExpenses.find(
	...
	async deleteExpense(expense: ProcessedExpenseItem): Promise<void> {
		try {
			...
			if (
				...
				doc.medical_expenses &&
				typeof expense.expenseIndex === "number"
			) {
	...
	filterExpenses(
		...
		return expenses
			.filter((expense) => {
				...
				if (
					filters.amountMin &&
					expense.amount < Number.parseFloat(filters.amountMin)
				) {
					return false
				}
				if (
					filters.amountMax &&
					expense.amount > Number.parseFloat(filters.amountMax)
				) {
					return false
				}
	...
	async getExpenseAnalytics(
		...
		try {
			...
			return {
				totalAmount,
				medicalAmount: medicalExpenses.reduce(
	...
	async getMedicalAnalytics(
		...
		try {
			...
			const directTotal = directMedical.reduce(
				(sum, exp) => sum + exp.amount,
				0,
			)
			const indirectTotal = indirectMedical.reduce(
				(sum, exp) => sum + exp.amount,
				0,
			)
			...
			const categoryBreakdown = {
				...
				indirect_medical: this.getCategoryBreakdown(
					indirectMedical,
					totalMedical,
				),
				other: [],
				...
				summary: {
					...
					average_per_expense:
						medicalExpenses.length > 0
							? totalMedical / medicalExpenses.length
							: 0,
				},
				insights: [] as any[],
				top_expenses: medicalExpenses
