...
export const CACHE_KEYS = {
	...
	// 🚀 NEW: Enhanced income analytics and filter cache keys
	INCOME_ANALYTICS: "income_analytics",
	INCOME_FILTER_OPTIONS: "income_filter_options",
	INCOME_PERIOD_INFO: "income_period_info",
	...
	getStorageUsage() {
		try {
			...
			const cacheKeys = keys.filter(
				(key) => key.startsWith(this.storagePrefix) || key === this.metaKey,
			)
	...
	window.debugCache = () => {
		...
		console.log(
			Object.keys(localStorage).filter(
				(key) => key.startsWith("artha_cache_") || key === "artha_cache_meta",
			),
		)
	}
}
